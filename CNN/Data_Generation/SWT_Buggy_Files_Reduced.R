...
# remove files with only 1 bug report linked.
Count <- rle( sort( SWT$files ) )
SWT$Count <- Count[[1]][ match( SWT$files , Count[[2]] ) ]
SWT<-SWT[! SWT$Count==1,]
...
write.table(data.frame(),file="/Volumes/CORSAIR/Results/SWT/Buggy_Files_Reduced/rt-polarity.neg",col.names = FALSE,row.names = FALSE,quote = FALSE)
write.table(data.frame(),file="/Volumes/CORSAIR/Results/SWT/Buggy_Files_Reduced/rt-polarity.pos",col.names = FALSE,row.names = FALSE,quote = FALSE)
write.table(data.frame(bug_id=as.integer(),sentence=as.character()),file="/Volumes/CORSAIR/Results/SWT/Buggy_Files_Reduced/bug_id_sentence",col.names = FALSE,row.names = FALSE,quote = FALSE)
...
for(i in 1:count_of_distinct_bug_reports){
  ...
  pos_file_path <- paste("/Volumes/CORSAIR/Results/SWT/Buggy_Files_Reduced/","rt-polarity.pos", sep="")
  neg_file_path <- paste("/Volumes/CORSAIR/Results/SWT/Buggy_Files_Reduced/","rt-polarity.neg", sep="")
  tot_file_path <- paste("/Volumes/CORSAIR/Results/SWT/Buggy_Files_Reduced/","bug_id_sentence", sep="")
