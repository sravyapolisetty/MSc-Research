## Script For Generating the Linked and Non-Linked Records For CNN Classification
## Source Files: Only Buggy Files(Files related to tests are removed)
## rt-polarity.pos : Linked records (bug report text and source file text combined)
## rt-polarity.pos : Non-Linked records(bug report text and source file text combined)
...
SWT_Linked_Files<-read.csv(file="/Volumes/CORSAIR/Results/SWT/SWT_Linked_file_data.txt",sep="\t",header = FALSE,col.names=c("file","file_text"))
...
SWT_Linked_Files<-SWT_Linked_Files[SWT_Linked_Files$file %in% SWT$files,]

SWT_All_Files<-read.csv(file="/Volumes/CORSAIR/Results/SWT/SWT_file_data.txt",sep="\t",header = FALSE,
                           col.names=c("file","file_text"))

SWT_All_Files$file<-gsub("\\./","",SWT_All_Files$file)
SWT_All_Files$file<-trimws(SWT_All_Files$file,which="both")
SWT_All_Files$file_text<-as.character(SWT_All_Files$file_text)
SWT_All_Files<-SWT_All_Files[!SWT_All_Files$file_text=="",]

# Separate Non-Linked Files
SWT_Non_Linked<-SWT_All_Files[! SWT_All_Files$file %in% SWT_Linked_Files$file,]

# Combining Linked and Non-Linked files
# This is the complete set of files in SWT repo.
SWT_Files<-rbind(SWT_Linked_Files,SWT_Non_Linked)

## remove all test case related files
SWT_Files<-SWT_Files[!grepl("tests\\/", SWT_Files$file),]
SWT_Files<-SWT_Files[!grepl("testdata\\/", SWT_Files$file),]
...
#write.table(dfBRRandom,file="/Volumes/CORSAIR/Results/SWT/SWT_dfBR_Long.txt",sep="\t", row.names = FALSE, col.names = FALSE)

# Shell script keeps only unique words in the bug reports.
...
write.table(data.frame(),file="/Volumes/CORSAIR/Results/SWT/All_Files/rt-polarity.pos",col.names = FALSE,row.names = FALSE,quote = FALSE)
write.table(data.frame(),file="/Volumes/CORSAIR/Results/SWT/All_Files/rt-polarity.neg",col.names = FALSE,row.names = FALSE,quote = FALSE)
write.table(data.frame(bug_id=as.integer(),sentence=as.character()),file="/Volumes/CORSAIR/Results/SWT/All_Files/bug_id_sentence",col.names = FALSE,row.names = FALSE,quote = FALSE)
...
for(i in 1:count_of_distinct_bug_reports){
  ...
  pos_file_path <- paste("/Volumes/CORSAIR/Results/SWT/All_Files/","rt-polarity.pos", sep="")
  neg_file_path <- paste("/Volumes/CORSAIR/Results/SWT/All_Files/","rt-polarity.neg", sep="")
  tot_file_path <- paste("/Volumes/CORSAIR/Results/SWT/All_Files/","bug_id_sentence", sep="")
