## NegRev: Filtered in MongoDB. Considering all cases which have atleast one flag named 'review' with value '-'.
## PosRev: Filtered in MongoDB. Considering only cases with positive review by all reviewers.
...
spark.df.selectedbugs<- read.df("",source ="com.mongodb.spark.sql.DefaultSource", 
                                uri = "mongodb://127.0.0.1/Mozilla.selected_bugs.bson")

spark.df.changeSet<-read.df("",source ="com.mongodb.spark.sql.DefaultSource", 
                            uri = "mongodb://127.0.0.1/Mozilla.selected_changeset.bson")

spark.df.PosRev<-read.df("",source ="com.mongodb.spark.sql.DefaultSource", 
                               uri = "mongodb://127.0.0.1/Mozilla.posRev")

spark.df.NegRev<-read.df("",source ="com.mongodb.spark.sql.DefaultSource", 
                         uri = "mongodb://127.0.0.1/Mozilla.NegRev")

nrow(spark.df.selectedbugs)
nrow(spark.df.NegRev)
nrow(spark.df.PosRev)
...
df.status<-sql("SELECT bugs.status,COUNT(*) AS COUNT FROM bugstable GROUP BY bugs.status")
...
#### Get Bug id, status, classification, cf_last_resolved, severity, creation_time,last_change_time
#### from selected bugs BSON
...
barplot(x,names.arg=year.labels,col=c(0,1),xlab="Year",ylab="Number of Patches",main = "Evolution Of Patches with Positive Reviews")
