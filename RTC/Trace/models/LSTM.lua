...
local LSTM, parent = torch.class('tracenn.LSTM', 'nn.Module')
...
function LSTM:forward(inputs, reverse)
  ...
  for t = 1, size do
    ...
    -- input:float() when using the embedding read from the util/glove.lua (in test_trace.lua file)
--    local outputs = cell:forward({input:float(), prev_output[1], prev_output[2]})
...
function LSTM:share(lstm, ...)
  ...
  if self.hidden_dim ~= lstm.hidden_dim then error("LSTM memory dimension mismatch") end
