...
private static class LoginHandler implements ILoginHandler, ILoginInfo {
	...
	private static void printResult(IQueryResult<IResolvedResult<IWorkItem>> result,ITeamRepository repo) throws TeamRepositoryException {
		...
		while (result.hasNext(null)) {
			...
			String testCase=getTestCases(resolved.getItem(),repo);
				...
				Element testCaseDOORS=doc.createElement("testcaseDOORS");
				testCaseDOORS.appendChild(doc.createTextNode(testCase));
...
private static String getTestCases(IWorkItem workItem, ITeamRepository repo){
	...
	try{
		...
		for (IEndPointDescriptor iEndPointDescriptor : endpoints){
			...
			if(iEndPointDescriptor.getLinkType().getLinkTypeId()==WorkItemLinkTypes.TESTED_BY_TEST_CASE)
			{
				
				 testCase= iEndPointDescriptor.getDisplayName();
				 break;
    ...
    for (ILink link: linkCollection) {
        ...
        System.out.println("\t" + link.getThisEndpointDescriptor(link.getTargetRef()).getDisplayName() + " " + link.getTargetRef().getComment());
